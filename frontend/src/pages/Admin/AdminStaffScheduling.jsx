...
export default function AdminStaffScheduling() {
    ...
    const stats = useMemo(() => {
        const totalCounter = demands.reduce((sum, item) => sum + (item.counterRequired || 0), 0);
        const totalCheckin = demands.reduce((sum, item) => sum + (item.checkinRequired || 0), 0);
        const totalConcession = demands.reduce((sum, item) => sum + (item.concessionRequired || 0), 0);
        const totalMulti = demands.reduce((sum, item) => sum + (item.multiRequired || 0), 0);
    ...
    const handleGenerateDemand = async () => {
        try {
            ...
            setSuccess("Đã tạo lại staffing demand từ showtime.");
            await loadPage(businessDate);
        } catch (err) {
            setError(getErrorMessage(err, "Không thể generate demand."));
        } finally {
            setActionKey("");
        }
    };
    ...
    const handleGeneratePlan = async () => {
        try {
            ...
            setSuccess("Đã tạo schedule plan tự động.");
            await loadPage(businessDate);
        } catch (err) {
            setError(getErrorMessage(err, "Không thể generate schedule plan."));
        } finally {
            setActionKey("");
        }
    };
        ...
        <div className="staff-scheduling-page">
            <div className="staff-scheduling-header">
                <div>
                    ...
                    <h1>Xếp lịch staff theo showtime</h1>
                    <p>
                        Generate demand từ lịch chiếu, tạo kế hoạch ca và publish lịch để nhân viên xác nhận.
                        Mô hình này bám theo tải vận hành thực tế thay vì chia ca cứng theo khung giờ.
                    </p>
                </div>
                <div className="staff-scheduling-toolbar">
                    ...
                    >
                        {actionKey === "generate-demand" ? "Đang xử lý..." : "Generate demand"}
                    </button>
                    <button
                        type="button"
                        className="secondary"
                    ...
                    >
                        {actionKey === "generate-plan" ? "Đang tạo..." : "Generate plan"}
                    </button>
                    {latestDraftPlan && (
                        <button
                            type="button"
            ...
            <div className="staff-scheduling-stats">
                ...
                <div className="staff-scheduling-stat-card">
                    <span>Plans / Published</span>
                    <strong>{stats.plans} / {stats.publishedPlans}</strong>
                </div>
                <div className="staff-scheduling-stat-card">
                    <span>Assignments / Confirmed</span>
                    <strong>{stats.assignments} / {stats.confirmedAssignments}</strong>
                </div>
            </div>
            ...
            <div className="staff-scheduling-grid">
                <section className="staff-scheduling-panel">
                    <div className="panel-header">
                        <h2>Nhu cầu theo window</h2>
                        <span>{demands.length} mốc</span>
                    </div>
                    {loading ? (
                        <div className="empty-state">Đang tải dữ liệu...</div>
                    ) : demands.length === 0 ? (
                        <div className="empty-state">Chưa có demand. Hãy generate từ showtime trước.</div>
                    ) : (
                        <div className="table-wrap">
                            <table>
                                ...
                                <tbody>
                                    {demands.map((item) => (
                                        <tr key={item.id}>
                                            <td>{formatRange(item.windowStart, item.windowEnd)}</td>
                                            <td>{item.counterRequired}</td>
                                            <td>{item.checkinRequired}</td>
                                            <td>{item.concessionRequired}</td>
                                            <td>{item.multiRequired}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            ...
                            {plans.map((plan) => (
                                <article key={plan.id} className={`plan-card ${plan.status?.toLowerCase() || ""}`}>
                                    <div className="plan-card-top">
                                        <div>
                                            <h3>Plan #{plan.id}</h3>
                                            <p>{plan.note || "Auto generated from staffing demand"}</p>
                                        </div>
